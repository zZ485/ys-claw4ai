...
class CrawlerParamsConfig:
    ...
    def get_max_crawlers(self, url_count: int) -> int:
        """根据URL数量获取最大并发爬虫实例数"""
        settings = self.get_config().get("concurrency_settings", {}).get("max_crawlers_by_url_count", [])
    ...
    def get_batch_size(self, url_count: int) -> int:
        """根据URL数量获取批量写入大小"""
        settings = self.get_config().get("batch_size_settings", {}).get("by_url_count", [])
        for setting in settings:
            if url_count <= setting.get("max_urls", 999999):
    ...
    def get_flush_interval(self, url_count: int) -> int:
        """根据URL数量获取刷新间隔"""
        settings = self.get_config().get("flush_interval_settings", {}).get("by_url_count", [])
        for setting in settings:
            if url_count <= setting.get("max_urls", 999999):
    ...
    def get_progress_settings(self) -> Dict[str, int]:
        """获取进度设置"""
        return self.get_config().get("progress_settings", {
            "progress_after_fetch": 20,
            "progress_after_crawl": 90,
            "progress_complete": 100
        })
    ...
    def get_error_handling_settings(self) -> Dict[str, Any]:
        """获取错误处理设置"""
        return self.get_config().get("error_handling", {
            "max_retry_attempts": 3,
            "retry_delay_seconds": 1.0,
            "log_success_threshold": 100
        })
