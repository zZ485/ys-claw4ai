    ...
    async def _periodic_flush(self):
        ...
        while not self._closed:
            try:
                ...
                async with self._lock:
                    # 检查是否需要刷新（基于时间间隔）
                    if self.buffer and (datetime.now() - self._last_flush_time).seconds >= self.flush_interval:
                        await self._flush_buffer()
...
class BatchWriterManager:
    ...
    async def get_writer(self, file_name: str, batch_size: int = 10, flush_interval: int = 30) -> BatchWriter:
        """
        获取或创建BatchWriter实例
        ...
        async with self._lock:
            if file_name not in self.writers:
                self.writers[file_name] = BatchWriter(file_name, batch_size, flush_interval)
