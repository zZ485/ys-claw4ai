...
LoggerConfig.setup_crawler_logger(
    log_file="ennews_newsflash", project_root=project_root
)
logger = logging.getLogger("ennews_newsflash")
    ...
    async with AsyncWebCrawler(config=browser_config) as crawler:
        ...
        if is_incremental:
            ...
            if latest_link:
                ...
                logger.info("增量模式：未找到最新链接，将生成所有链接")
        ...
        while should_continue and current_id > 0:
            ...
            if is_incremental and latest_link and link == latest_link:
                logger.info(f"增量模式：已达到最新链接 {latest_link}，停止生成")
            ...
            if not is_incremental and current_id == 1:
                logger.info("全量模式：已生成到 news-1.html，停止生成")
                should_continue = False
                break
...
async def main():
    ...
    logger.info(f"获取到最新快讯ID: {result.get('latest_id', 'N/A')}")
    logger.info(f"总共生成 {result['count']} 个链接")
