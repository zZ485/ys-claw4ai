...
async def fetch_all_news(max_pages=5, use_proxy=False, proxy_list=None):
    """
    获取多页新闻，返回完整URL列表
    ...
    print(f"共获取 {len(all_urls)} 条新闻URL")
    return all_urls
...
async def get_links(use_proxy=False, proxy_list=None, max_pages=2):
    ...
    # 获取所有URL
    all_urls = await fetch_all_news(max_pages=max_pages, use_proxy=use_proxy, proxy_list=proxy_list)
