...
- `cleaning_config`: 清洗配置，JSON对象，用于控制采集内容的后处理净化策略。系统采用**两阶段清洗架构**，先由 Crawl4AI 爬取引擎完成结构化提取，再由后处理引擎执行正则驱动的语义级净化，两个阶段协同工作，确保最终输出内容精炼、噪声极低。包含以下字段：
  - `source`: 是否清洗来源声明，0表示不清洗，1表示清洗，默认为1
    - 基于多模式正则引擎，精准识别并移除正文中的来源声明行，覆盖模式包括：`文章来源：xxx`、`来源：xxx`、`本文来自：xxx`、独立观点声明（如"本文为作者独立观点，不代表xxx立场"）等
  - `image_source`: 是否清洗图源标注，0表示不清洗，1表示清洗，默认为1
    - 自动识别并移除正文中的图片来源标注行，覆盖模式包括：`图源：xxx`、`图片来源：xxx`、`图片来自：xxx`
  - `author`: 是否清洗作者署名，0表示不清洗，1表示清洗，默认为1
    - 自动识别并移除正文中的作者署名行，覆盖模式包括：`作者：xxx`、`文/xxx`、`撰文：xxx`、`记者xxx`
  - 清洗后自动执行冗余空行压缩与首尾空白裁剪，保证输出排版整洁

> **两阶段清洗架构说明**
>
> 本系统的内容清洗分为两个阶段，贯穿采集全链路：
>
> | 阶段 | 执行时机 | 引擎 | 处理内容 |
> |------|---------|------|---------|
> | **第一阶段：结构化提取** | Crawl4AI 爬取时 | PruningContentFilter（阈值 0.48） | 自动剔除导航栏、侧边栏、广告、页脚、Cookie 提示等页面噪声元素；排除 `<img>` 标签（图片标签），输出纯文本 Markdown；仅保留文本密度高于阈值的正文区块 |
> | **第二阶段：语义净化** | 数据写入前 | ContentCleaner（正则引擎） | 按上述 `cleaning_config` 配置，逐项精准移除来源声明、图源标注、作者署名等语义噪声；最后执行空行压缩与首尾裁剪 |
>
> 两个阶段互补配合：第一阶段从 HTML 结构层面过滤页面噪声，第二阶段从文本语义层面消除内容噪声，确保最终输出即为高信噪比的核心正文。
