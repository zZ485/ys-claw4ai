...
class CustomsRegulationCrawler:
    ...
    async def crawl_regulations_batch(self, urls: List[str]) -> List[Dict]:
        ...
        try:
            ...
            async with AsyncWebCrawler(config=browser_config) as crawler:
                ...
                print(f"使用单个爬虫实例分批爬取 {len(urls)} 个链接，每批 {batch_size} 个")
                
                for batch_idx, batch in enumerate(url_batches):
                    print(f"正在处理第 {batch_idx + 1}/{len(url_batches)} 批，包含 {len(batch)} 个链接")
                    
                    # 使用arun_many方法批量爬取
                    batch_results = await crawler.arun_many(urls=batch, config=crawler_config)
                    ...
                    for result in batch_results:
                        ...
                        document_number = doc_no_match.group(1).strip() if doc_no_match else ""
                        
                        # 解析发布日期
                        pub_date_match = re.search(r'发布[日期:：]\s*([^\n]+)', fit_markdown)
                        publish_date = pub_date_match.group(1).strip() if pub_date_match else ""
                        # 清理发布日期，移除可能的"期】"前缀
                        publish_date = re.sub(r'^期】', '', publish_date)
                        
                        # 解析实施日期
                        eff_date_match = re.search(r'实施[日期:：]\s*([^\n]+)', fit_markdown)
                        effective_date = eff_date_match.group(1).strip() if eff_date_match else ""
                        
                        # 解析法规类型
                        reg_type_match = re.search(r'(部门规章|规范性文件|其他)', fit_markdown)
                        regulation_type = reg_type_match.group(1).strip() if reg_type_match else ""
                        
                        # 解析效力状态
                        status_match = re.search(r'(有效|现行|实施中|失效|废止)', fit_markdown)
    ...
    def parse_regulation_info(self, url: str, data: Dict) -> RegulationInfo:
        ...
        if isinstance(title_data, dict):
            ...
            if not title and "regex" in title_data:
                match = re.search(title_data.get("regex", ""), title_data.get("text", ""))
                if match:
    ...
    def filter_valid_regulations(self, regulations: List[RegulationInfo]) -> List[RegulationInfo]:
        """按【效力】筛选有效法规"""
        valid_regulations = []
        for regulation in regulations:
            # 如果状态字段包含"有效"、"现行"或"实施中"，认为是有效法规
            if any(keyword in regulation.status for keyword in ["有效", "现行", "实施中"]):
                valid_regulations.append(regulation)
        return valid_regulations
    
    def classify_regulations(self, regulations: List[RegulationInfo]) -> Dict[str, List[RegulationInfo]]:
        """按【法规类型】分类法规"""
        classified = {
            "部门规章": [],
            "规范性文件": [],
    ...
    def export_to_excel(self, classified_regulations: Dict[str, List[RegulationInfo]], output_file: str = "customs_regulations.xlsx"):
